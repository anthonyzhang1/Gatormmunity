/* This file handles the display of the Create Listing page. Here, the user can create listings by filling out a form. */
...
import { howToCreateListing, meetingBuyerSeller } from '../components/help/Questions';
...
export default function CreateListing() {
    ...
    const questions = [howToCreateListing, meetingBuyerSeller];
    ...
    const [form, setForm] = useState({
        ...
        listingDescription: "",
        price: "",
        category: LISTING_CATEGORIES[4] // the default category (miscellaneous)
    });
    ...
    function createListing(e) {
        ...
        /** The form that we send to the back end containing the listing's data. */
    ...
    /* On successful listing creation, inform the user of such and redirect them to the newly created listing's page.
     * On error, show an error message. */
    useEffect(() => {
        if (returnData?.status === SUCCESS_STATUS) {
            alert("Listing created!");
            navigate(`/listing/${returnData.listingId}`);
