 ...
 * listings first. The user can specify what they want to search for with the search options on the left, which contains
 * filters and type of item to search for, e.g. users or threads. */
...
export default function Search() {
    ...
    const [roleFilter, setRoleFilter] = useState(ANY_CATEGORY); // for user search
    const [listingCategoryFilter, setListingCategoryFilter] = useState(ANY_CATEGORY); // for listing search
    const [maxPriceFilter, setMaxPriceFilter] = useState(""); // for listing search
    const [threadCategoryFilter, setThreadCategoryFilter] = useState(ANY_CATEGORY); // for thread search

    const [returnData, setReturnData] = useState(null); // stores the data being shown
    const [currentlyShowing, setCurrentlyShowing] = useState({ // stores what is currently being displayed
        type: LISTINGS, // listings, by default
        dropdownFilter: ANY_CATEGORY
    });

    /** 
     * Send the search terms and the role filter to the back end so it can search the database for users.
     * 
    ...
    function searchUsers(e) {
        ...
        setCurrentlyShowing({ // set loading message
            type: LOADING,
            dropdownFilter: currentlyShowing.dropdownFilter
        });
        ...
        fetch('/api/users/search', {
            ...
            .then(data => {
                setCurrentlyShowing({
                    type: USERS,
                    dropdownFilter: getRoleDescription(roleFilter)
                });
                setReturnData(data);
            })
            .catch(console.log());
    ...
    /** 
     * Send the search terms, the category filter, and the max price filter to the back end
     * so it can search the database for listings.
     * 
     * Fetch Request's Body:
    ...
    function searchListings(e) {
        ...
        fetch('/api/listings/search', {
            ...
            .then(data => {
                setCurrentlyShowing({
                    type: LISTINGS,
                    dropdownFilter: listingCategoryFilter,
    ...
    /** 
     * Send the search terms and the category filter to the back end so it can search the database for threads.
     * 
     * Fetch Request's Body:
    ...
    function searchThreads(e) {
        ...
        fetch('/api/threads/search', {
            ...
            .then(data => {
                setCurrentlyShowing({
                    type: THREADS,
                    dropdownFilter: threadCategoryFilter
                });
                setReturnData(data);
            })
            .catch(console.log());
    ...
    function getSearchResultsTitle() {
        /** The message that will be returned and displayed for the search results title. */
        let title = '';

        switch (currentlyShowing.type) {
            case ERROR: return returnData?.message;
            case LOADING: return 'Searching...';
            case USERS:
                title = `Currently Searching: Users
                         Found ${returnData?.numUsersMatched}`; // number of matches

                if (currentlyShowing.dropdownFilter === ANY_CATEGORY) title += ` user(s)`; // category
                else title += ` ${currentlyShowing.dropdownFilter}(s)`;

                if (searchTerms) return title += ` with "${searchTerms}".`; // search terms
                else return title += '.';

            case LISTINGS:
                title = `Currently Searching: Listings
                         Found ${returnData?.numListingsMatched}`; // number of matches

                if (currentlyShowing.dropdownFilter === ANY_CATEGORY) title += ` listing(s)`; // category
                else title += ` ${currentlyShowing.dropdownFilter} listing(s)`;
            ...
            case THREADS:
                title = `Currently Searching: Threads
                         Found ${returnData?.numThreadsMatched}`; // number of matches

                if (currentlyShowing.dropdownFilter === ANY_CATEGORY) title += ` thread(s)`; // category
                else title += ` ${currentlyShowing.dropdownFilter} thread(s)`;
    ...
    /* Call the search listings function the first time this page is navigated to. */
    useEffect(() => searchListings(), []); // eslint-disable-line react-hooks/exhaustive-deps

    /* Searches in the same category but with different search terms when the user enters new search terms in the search bar. */
    useEffect(() => {
        switch (currentlyShowing.type) {
            case USERS:
                searchUsers();
    ...
    useEffect(() => {
        if (returnData?.status === ERROR_STATUS) {
            setCurrentlyShowing({ // show error screen
                type: ERROR,
                dropdownFilter: currentlyShowing.dropdownFilter
            });
        }
    ...
    if (!userSession) return; // wait for fetch
        ...
        <div className="search-page">
            <Container className="search-container" fluid>
                <Row>
                    {/* The left column with the search options. */}
                    <Col className="search-filters-col" md="auto">
                        <h3 className="filters-header">Search Options</h3>

                        {/* Must be logged in to search for users. */}
                        {userSession.isLoggedIn && <Form className="search-users-form" onSubmit={searchUsers}>
                            ...
                            {/* Apply the filters when pressing the search button.
                              * If users are currently being shown, highlight the button. */}
                            <Button className={`search-button search-users-button
                                ${currentlyShowing.type === USERS ? "default-button-color" : "btn-secondary"}`} type="submit"
                            >
                                Search Users
                            </Button>
                        </Form>}

                        <Form className="search-listings-form" onSubmit={searchListings}>
                            ...
                            {/* Apply the filters when pressing the search button.
                              * If listings are currently being shown, highlight the button. */}
                            <Button className={`search-button search-listings-button
                                ${currentlyShowing.type === LISTINGS ? "default-button-color" : "btn-secondary"}`} type="submit"
                            >
                                Search Listings
                            </Button>
                        </Form>
                        ...
                        {userSession.isLoggedIn && <Form className="search-threads-form" onSubmit={searchThreads}>
                            ...
                            {/* Apply the filters when pressing the search button.
                              * If threads are currently being shown, highlight the button. */}
                            <Button className={`search-button search-threads-button
                                ${currentlyShowing.type === THREADS ? "default-button-color" : "btn-secondary"}`} type="submit"
                            >
                                Search Threads
                            </Button>
                        </Form>}
                    ...
                    {/* The column with the search results in it. */}
