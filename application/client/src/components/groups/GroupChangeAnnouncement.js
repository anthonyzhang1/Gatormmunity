/* This file holds the component which displays a modal for changing the announcement of a group.
...
export default function GroupChangeAnnouncementModal(props) {
    ...
    /** 
     * Sends the new announcement's text to the back end for them to handle.
     * 
     * Fetch Request's Body:
     * announcement: {string} The new announcement's text. Must be 0-5'000 characters long.
     ...
     */
    function changeAnnouncement(e) {
        e.preventDefault();

        fetch('/api/groups/change-announcement', {
            method: 'PUT',
    ...
    /* Refresh the page on successful announcement change, or show an error if an error occurred. */
    useEffect(() => {
