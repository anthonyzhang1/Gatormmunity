/* This file holds the component which displays a modal for changing the user's profile picture.
 * The component is called via the User's Profile page. */
...
export default function ChangeProfilePictureModal(props) {
    ...
    const [returnData, setReturnData] = useState(null); // stores the data sent from the back end
    ...
    function changeProfilePicture(e) {
        ...
        fetch('/api/users/change-profile-picture', {
            method: 'POST',
            body: formData
        })
            .then(res => res.json())
    ...
    /* Refreshes the user's profile page on successful picture change. On error, show an error. */
    useEffect(() => {
